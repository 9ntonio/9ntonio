...
async function cleanupTempDirectories() {
	await Promise.all([fs.remove(DIRECTORIES.MODERN_TEMP), fs.remove(DIRECTORIES.LEGACY_TEMP)]);
}
...
function printFeaturesAnalysis(modernStats, legacyStats) {
	...
	const allFeatures = new Set([...Object.keys(modernStats.featureStats), ...Object.keys(legacyStats.featureStats)]);
